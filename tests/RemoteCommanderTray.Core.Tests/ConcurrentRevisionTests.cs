...
public sealed class ConcurrentRevisionTests : IAsyncLifetime
{
    ...
    public ConcurrentRevisionTests()
    {
    ...
    public async Task Old_auth_output_already_queued_cannot_force_replacement_to_sign_in()
    {
        ...
        _factory.Latest.Emit("Device ready:");
        await Until(() => _supervisor.Snapshot.State == AgentState.Online);
        Assert.InRange(_notices.Count, 0, 1); // Old current generation may notify before Restart acquires the lock.
    ...
    [Fact]
    public async Task Cleanup_failure_blocks_replacement_until_tray_is_restarted()
    {
        await _supervisor.StartAsync();
        var old = _factory.Latest;
    ...
    public void Unrecognized_startup_data_is_never_assumed_enabled()
    {
        ...
        Assert.Equal(StartupState.Unknown, StartupApproval.Resolve(true, new byte[] { 255 }));
    }
}
