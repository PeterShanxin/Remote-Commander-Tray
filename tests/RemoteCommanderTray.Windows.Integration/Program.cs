...
internal static class Program
{
    ...
    private static async Task<int> Helper(string[] args)
    {
        switch (args[0])
        {
            ...
            case "--output":
                ...
                Console.WriteLine($"console={GetConsoleWindow()};color={Environment.GetEnvironmentVariable("FORCE_COLOR")}");
    ...
    private static async Task OutputAndEnvironment()
    {
        ...
        Require(lines.Contains("console=0;color=0"), "The agent received a console window or wrong environment.");
    ...
    private static async Task BoundedOutput()
    {
        ...
        Require(result.Succeeded && result.Output.Contains("oversized agent line omitted") && result.Output.Contains("Device ready:"),
    ...
    private static Task StartupRecords()
    {
        ...
        try
        {
            ...
            Require(StartupRegistration.ReadState(Registry.CurrentUser, runPath, approvalPath, "Probe") == StartupState.DisabledByWindows,
            ...
            Require(StartupRegistration.ReadState(Registry.CurrentUser, runPath, approvalPath, "Probe") == StartupState.Unknown,
                "Malformed approval must not be treated as enabled.");
            run.DeleteValue("Probe");
            Require(StartupRegistration.ReadState(Registry.CurrentUser, runPath, approvalPath, "Probe") == StartupState.NotRegistered,
